                            ...
                            <div className="fixed bg-black/60 backdrop-blur-sm flex items-center justify-center inset-0 z-99999">
                                <div className="relative bg-white shadow-lg p-6 rounded-xl w-[90%] max-w-xl overflow-y-auto max-h-[90vh]">
                                    ...
                                    <form onSubmit={handleSubmit} ref={formRef}>
                                        ...
                                        <Select id="payment_method" value={orderForm.payment_method} label="Payment Method" required onChange={(val) => setOrderForm({ ...orderForm, payment_method: val })} className="text-black">
                                            <option value="">--- Select Payment ---</option>
                                            <option value="CASH">CASH</option>
                                            <option value="QRIS">QRIS</option>
                                        </Select>
                                        <Select id="status" value={orderForm.status} label="Status" required onChange={(val) => setOrderForm({ ...orderForm, status: val })} className="text-black">
                                            <option value="">--- Select Status ---</option>
                                            <option value="NEW">NEW</option>
                                            <option value="PAID">PAID</option>
                                            <option value="DONE">DONE</option>
                                        </Select>
                                        <InputGroupComponent id="order-note" type="text" value={orderNote} onChange={(val) => setOrderNote(val)} label="Order Note" className="text-black" />
                                        <div className="flex justify-end gap-2 mt-4">
                                            <ButtonDanger type="button" onClick={() => setOrder(false)}>Cancel</ButtonDanger>
